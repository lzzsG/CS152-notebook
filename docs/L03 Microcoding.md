...
- **LW0**：
  - **Address (Inputs)**: `μPC = LW0`，执行内存地址的计算。
  - **Control Lines**: `MA := A + B`，基址寄存器和偏移量相加，计算内存地址。
  - **Next μPC**: `LW1`，进入加载数据阶段。

- **LW1**：
  - **Address (Inputs)**: `μPC = LW1`，等待内存返回数据。
  - **Control Lines**: `wait for memory`，处理器等待内存读取完成。
  - **Next μPC**: `LW2`，准备将内存数据写入目标寄存器。

- **LW2**：
  - **Address (Inputs)**: `μPC = LW2`，将从内存中读取的数据写入寄存器 `rd`。
  - **Control Lines**: `Reg[rd] := Mem`，内存返回的数据写入寄存器 `rd`。
  - **Next μPC**: `fetch0`，回到取指阶段，准备执行下一条指令。
...
每条微指令从 ROM 中读取时，会输出相应的控制信号，这些信号用于控制处理器各个功能单元的操作。例如，控制信号可能包括设置寄存器值、启动 ALU 运算、访问内存等。

---

## Encoded ROM Contents

ROM 的内容以真值表的形式呈现，每个微程序计数器（μPC）对应特定的控制信号输出和下一步跳转信息。以下是真值表中的一些典型例子，展示了 RISC-V 指令执行的几个阶段的微指令控制。
...
通过这种单总线微代码引擎，RISC-V 处理器能够灵活高效地执行各种指令。微指令控制器使用 ROM 存储控制信号和跳转逻辑，每条微指令通过 μPC 来控制处理器的不同阶段操作。
...
- **例子**：Motorola 68000 微处理器使用了纳米编码，其中 17 位的微指令包含一个 10 位的微跳转（μjump）或者一个 9 位的纳米指令指针。这些纳米指令是 68 位宽的，能够解码为 196 个控制信号。
...
### 实验室小组匹配
- 学生需填写以下表格进行实验室小组匹配：
  - [实验室匹配表单链接](https://docs.google.com/forms/d/e/1FAIpQLSfxXVEwM6a-pR2-RU_ntjD1zfskipOWf4e-8eCljxfOHtTiaA/viewform?usp=sf_link)
