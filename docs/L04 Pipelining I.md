...
在流水线中，指令可能会相互依赖，导致数据冒险。在处理这些冒险时，有两种常见的策略：**互锁（Interlocking）** 和 **旁路（Bypassing）**。
...
### 旁路路径说明：
- 例如，当 `add` 指令的结果尚未写回寄存器文件时，旁路路径（红色箭头）可以将 ALU 的计算结果直接传递给下一条指令的输入端口（即寄存器 `A` 或 `B`），使得 `sub` 指令无需等待写回阶段的完成即可使用该结果。
...
**完全旁路的数据路径** 展示了流水线处理器如何利用旁路技术，避免流水线停顿，并提高指令执行的效率。在该设计中，ALU 的计算结果可以在执行阶段直接转发给下一条指令，而不必等待写回阶段完成。

### 数据路径说明：
- **红色路径** 表示旁路路径，当一条指令在 ALU 计算出结果后，结果可以通过旁路路径直接传递给后续指令的 ALU 输入端，绕过寄存器文件的写回和读取过程。
- 这种设计使得流水线能够以更高的并行性运行，减少了数据冒险导致的延迟。

### 旁路场景中的流水线图：
- 流水线图中的红色箭头显示了一条指令执行的结果被快速转发给下一条指令，而不是等待整个写回周期的完成。这避免了传统流水线中数据冒险引发的气泡，从而提高了流水线的整体效率。
...
4. **Kill Bit**：在跳转后，跳转指令后面已经进入流水线的指令将被“杀死”，即转换为气泡，避免对系统状态产生影响。
...
### 作业和实验安排
- **HW1 发布**：截止日期为 2 月 2 日。
- **Lab1 发布**：截止日期为 2 月 9 日。
- **Lab 分组**：通过提供的链接进行实验分组匹配。
- **讨论与办公时间**：从本周开始，查看课程日历以获取详细信息。
...
- **转移控制**：处理器会关闭其他中断并将控制权转交给运行在监督模式下的中断处理程序，负责处理此次中断。



## Trap: Altering the Normal Flow of Control

**陷阱（Trap）** 是指处理器因某种外部或内部事件而强制改变当前程序的控制流，从而跳转到特定的陷阱处理程序。陷阱通常是由程序无法预期或很少发生的事件引起的，典型例子包括异常或中断。正如图中所示，在指令 `I_i` 处发生了异常或中断，控制流转移到相应的陷阱处理程序 `H_i`，在处理完陷阱后再返回继续执行程序中的 `I_i+1`。
...
MIT 材料来源于 6.823 课程，UCB 材料来源于 CS252 课程。